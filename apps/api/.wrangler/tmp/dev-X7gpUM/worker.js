...
// .wrangler/tmp/bundle-cecqC7/checked-fetch.js
  ...
  async createAccount(account) {
    ...
    try {
      const result = await this.db.prepare(sql).bind(
        ...
        account.name,
        "SECURITIES",
        // Default account type (deprecated)
...
async function accountsHandler(request, env) {
  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      if (path2.endsWith("/default/current")) {
      ...
      if (path2.endsWith("/active/list")) {
        const accounts2 = await database.getAccounts();
        return new Response(
          JSON.stringify({
            success: true,
            data: accounts2,
      ...
      if (path2.includes("/by-tags")) {
        const url3 = new URL(request.url);
        const tagsParam = url3.searchParams.get("tags");
        if (!tagsParam) {
          return new Response(
            JSON.stringify({
              success: false,
        ...
        const accounts2 = await database.getAccountsByTags(tagNames);
        return new Response(
          JSON.stringify({
            success: true,
            data: accounts2,
...
// .wrangler/tmp/bundle-cecqC7/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cecqC7/middleware-loader.entry.ts
